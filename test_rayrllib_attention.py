...
ppo_config = (
    ...
    .env_runners(
        num_env_runners=8,  # Replaces num_rollout_workers
        num_envs_per_env_runner=4,  # Replaces num_envs_per_worker
    ...
    .training(
        model = {
            "use_attention": True,
            # "max_seq_len":6,  # Keeps the maximum sequence length 
            # "attention_num_transformer_units": 1,  # Keeps a single transformer unit
            # "attention_dim": 256, #the critic hidden layer size
            # "attention_memory_inference":6,  # Keeps the memory for inference 
            # "attention_memory_training": 6,  # Keeps memory for training 
            # "attention_num_heads": 4,  # number of attention heads 
            # "attention_head_dim": 64,  # the head dimension 
            # "attention_position_wise_mlp_dim": 256,  # MLP hidden dimension
...
checkpoint_dir = "/Users/sakshisharma/Desktop/rl_scheduling/tensorboard_logs/attention/PPO_Training_Experiment/PPO_MRLEnvironment_d07ca_00000_0_2025-02-05_16-35-23/checkpoint_000099" 
...
for episode in range(1):  # You can change the number of episodes for testing
    ...
    while not all(done.values()):
        ...
        for agent in marl_env.agents:
            # Compute action
            action[agent], state_out, _ = policy[agent].compute_single_action(
                obs[agent], state ,full_fetch=True
            )
...
action_df.to_csv('actions_test_attention.csv')

util.compute_and_save_total_cost(factory.consumption_dict,[price/1000 for price in price_signal[:-1]],output_filename="total_cost_attention.csv")
